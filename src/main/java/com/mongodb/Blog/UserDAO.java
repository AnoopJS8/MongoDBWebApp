...
public class UserDAO {
	...
	public Document validateLogin(String username, String password) {
		...
		/*
		 * List<Document> all= usersCollection.find().into(new
		 * ArrayList<Document>()); for(Document cur:all) {
		 * if(cur.getString("_id").equals(username)) { user=cur;break; } }
		 */
		MongoCursor<Document> cursor = usersCollection.find().iterator();
		try {
			while (cursor.hasNext()) {
				Document doc = cursor.next();
				if(doc.get("_id").toString().equals(username)){	//doc.getString("_id").equals(username)){
					user=doc;
		...
		// user=usersCollection.findOneAndReplace(arg0, arg1)//(new
